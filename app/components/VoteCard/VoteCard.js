...
function VoteCard({ gameId, yesCards, setPlayState }) {
  ...
  const handleCardClick = async (cardId) => {
    ...
    setVoteCardState({ voteRandomIndex: ArrayMethods.getRandomIndex(tempYesCards) })
  };

  const terminateVote = async (cardId) => {
    // const newYesCardIndexInYesCards = yesCards.findIndex(card => card._id === newYesCard._id);
    const voteRandomIndexCardInYesCards = yesCards.findIndex(card => card._id === yesCards[voteRandomIndex]._id);

    let newlyGeneratedYesCards;

    if (cardId === newYesCard._id) {
      // Insert newYesCard before the index of voteRandomIndex
      newlyGeneratedYesCards = ArrayMethods.insertItem(yesCards.slice(1), (voteRandomIndexCardInYesCards - 1), newYesCard);
    } else {
      // Insert newYesCard after the index of voteRandomIndex
      newlyGeneratedYesCards = ArrayMethods.insertItem(yesCards.slice(1), (voteRandomIndexCardInYesCards), newYesCard);
    }

    await gameService.updateYesCards(gameId, { cardIds: newlyGeneratedYesCards.map(card => card._id), eloScores: newlyGeneratedYesCards.map(card => card.eloRating) })
  ...
  return (
    <>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-8">
        ...
        <div className="cursor-pointer" onClick={() => handleCardClick(tempYesCards[voteRandomIndex]._id)} >
          <div className="h-52 w-52 md:h-[30vw] md:w-[30vw] relative mx-auto">
            <Image
